...
This project automates the provisioning of an **Azure Functions** using Terraform. It integrates with **GitHub Actions** to create a CI/CD pipeline that applies infrastructure changes upon pushing to the `main` branch. Also displays the cost of resources used using INFRACOST and creates the resources only after the approval from reviewers.
...
```bash
terraform apply \
  -var="resource_group_name=Functions-rg" \
  -var="resource_group_location=West Europe" \
  -var="storage_account_name=functionsstorage" \
  -var="storage_account_tier=Standard" \
  -var="storage_account_replication_type=LRS" \
  -var="app_service_plan_name=functions-service-plan" \
  -var="app_service_plan_sku_tier=Standard" \
  -var="app_service_plan_sku_size=S1" \
  -var="function_app_name=functionsapp"
...
| `Storage Account`         | Generated Storage account                            |
| `Service Plan`            | Generated Service Plan                               |
| `Functions App`           | Generated Functions                                  |
---
